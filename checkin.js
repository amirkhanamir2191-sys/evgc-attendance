...
export default async function handler(req, res) {
  ...
  if (distance > RADIUS_METERS) {
    return res.json({
      success: false,
      message: `You are too far from the venue (${Math.round(distance)}m away). Check-in requires being within ${RADIUS_METERS}m.`
    });
  }
  ...
  try {
    ...
    if (p.date_iso !== todayISO) {
      return res.json({
        success: false,
        message: `${p.name}, you are scheduled for ${p.batch} (${p.scheduled_date}), not today.`
      });
    }
    ...
    if (!existing.checkout_time) {
      ...
      const { error } = await supabaseAdmin
        .from('attendance')
        .update({ checkout_time: now, checkout_distance_m: Math.round(distance) })
        .eq('id', existing.id);
      if (error) throw error;
      return res.json({
        success: true,
    ...
    return res.json({
      success: false,
      action: 'complete',
      message: `${p.name}, you have already checked in and checked out today.`,
      checkinTime: fmt(existing.checkin_time),
