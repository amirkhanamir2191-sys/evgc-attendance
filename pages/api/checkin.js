...
export default async function handler(req, res) {
  ...
  const { loginId, lat, lon } = req.body;
  ...
  if (distance > RADIUS_METERS) {
    return res.json({ success: false, message: 'You are too far from the venue (' + Math.round(distance) + 'm away). Check-in requires being within ' + RADIUS_METERS + 'm.' });
  }
  try {
    ...
    if (matches.length === 0) return res.json({ success: false, message: 'ID not found.' });
    if (matches.length > 1) return res.json({ success: false, message: 'Multiple records found. Use your School ID.' });
    const p = matches[0];
    ...
    if (!existing) {
      ...
      const { error } = await supabaseAdmin.from('attendance').insert({ evgc_id: p.evgc_id, school_id: p.school_id, name: p.name, district: p.district, zone: p.zone, school_name: p.school_name, batch: p.batch, checkin_time: now, checkin_distance_m: Math.round(distance) });
    ...
    if (!existing.checkout_time) {
      const now = new Date().toISOString();
      const { error } = await supabaseAdmin.from('attendance').update({ checkout_time: now, checkout_distance_m: Math.round(distance) }).eq('id', existing.id);
      if (error) throw error;
